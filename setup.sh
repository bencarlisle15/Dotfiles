...
LINE=$((LINE+1));cat ~/Dotfiles/dconf-settings.ini | dconf load / || { echo $LINE ; exit 1; };
LINE=$((LINE+1));sudo update-grub || { echo $LINE ; exit 1; };
LINE=$((LINE+1));sudo apt install -y golang gnome-tweaks net-tools virtualbox wireshark adb fastboot libreoffice python3 python3-pip nmap exiftool neofetch docker docker-compose docker.io cheese aircrack-ng clamav|| { echo $LINE ; exit 1; };
LINE=$((LINE+1));flatpak install org.chromium.Chromium/ pinta com.microsoft.teams androidstudio intellij-idea-community signal com.visualstudio.code/ -y || { echo $LINE ; exit 1; };
LINE=$((LINE+1));cd ~ || { echo $LINE ; exit 1; };
LINE=$((LINE+1));mkdir -p ~/.config/linux-bing-wallpaper/ || { echo $LINE ; exit 1; };
